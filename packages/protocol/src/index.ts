...
export const NativeProbeStatusSchema = NativeStatusMetadataSchema.extend({
  ...
  /**
   * Broker-derived, never agent-reported: the first time this probe was seen
   * armed. `status` holds only the newest transition, so a probe that armed
   * and immediately reached its hit limit would otherwise be indistinguishable
   * from one that never armed. It is therefore absent from the metadata schema
   * that validates inbound native status events.
   */
